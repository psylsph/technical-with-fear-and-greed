"""
Live and test trading engine - Fixed version
"""
...
from ..config import BEST_PARAMS, PROJECT_ROOT, TEST_STATE_FILE, CACHE_DIR
...
# Database for storing live prices
LIVE_PRICES_DB = os.path.join(PROJECT_ROOT, "live_prices.db")
...
def init_live_prices_db():
    """Initialize live prices database."""
    os.makedirs(CACHE_DIR, exist_ok=True)
    conn = sqlite3.connect(LIVE_PRICES_DB)
    cursor = conn.cursor()
    cursor.execute(
...
def save_live_price(symbol: str, price: float, source: str = "unified"):
    ...
    try:
        ...
        cursor.execute(
            """INSERT OR REPLACE INTO prices (timestamp, symbol, price, source)
               VALUES (?, ?, ?, ?)""",
        ...
        print(f"Error saving live price: {e}")


def get_live_price_history(symbol: str, limit: int = 100) -> pd.DataFrame:
    """Get live price history from database."""
    try:
        conn = sqlite3.connect(LIVE_PRICES_DB)
        query = """
            SELECT timestamp, symbol, price, source
            FROM prices
            WHERE symbol = ?
        ...
        print(f"Error getting live price history: {e}")
        return pd.DataFrame()
...
def analyze_live_signal(
    ...
    Args:
        ...
        trading_client: Alpaca trading client (not used for signal gen)
    """
    try:
        # Use cached data for signal generation
        ...
        print(f"Fetching {symbol} data for signal generation...")
        if symbol == "ETH-USD":
        ...
        if ohlcv is None or len(ohlcv) < 30:
            print(f"Could not fetch sufficient {symbol} data (need at least 30 bars)")
            return None

        close_series = ohlcv["close"]
        print(f"Using cached data: {len(close_series)} bars")
...
def calculate_kelly_fraction(
    ...
    kelly_fraction *= 0.5
    return max(kelly_fraction, 0.01)
...
def should_trade(
    ...
    if signal == "buy" and current_position == 0:
        ...
        quantity = (max_position_value / price) * 0.95
        quantity = round(quantity, 6)
        return ("sell", max(quantity, 0.0001))
...
def run_test_trading(fgi_df: pd.DataFrame):
    ...
    try:
        while True:
            ...
            print(f"\nSleeping {CHECK_INTERVAL} seconds until next check...")
...
def run_live_trading(fgi_df: pd.DataFrame):
    ...
    try:
        ...
        print(f"Prices stored to: {LIVE_PRICES_DB}")
    except Exception as e:
    ...
    try:
        while True:
            try:
                ...
                # Get current price from unified fetcher (works reliably)
                current_price = get_current_price(SYMBOL)
                if current_price:
                    # Save to database for historical tracking
                    save_live_price(SYMBOL, current_price, "unified")
                    if not QUIET_MODE:
                ...
                # Get signal (uses cached data for RSI/indicators)
                signal_info = analyze_live_signal(fgi_df, SYMBOL, TRADING_CLIENT)
                if signal_info and "indicators" in signal_info:
                    ind = signal_info["indicators"]
                    # Use live price if available, otherwise cached price
                    price = current_price if current_price else ind.get("price", 0)
                    pnl = (price * position) - (position * 3000) if position > 0 else 0
            ...
            print(
                f"\nSleeping {CHECK_INTERVAL} seconds until next check ({CHECK_INTERVAL // 60} minutes)..."
            )
            time.sleep(CHECK_INTERVAL)
